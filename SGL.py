...
#        t_stack = tf.stack([t_ for _ in range(gl_op.get_shape()[-1])], -1)
        gl_plus = tf.cast((tf.abs(gl_op)-e_param * elayerwise[vind])> 0, tf.float32) * (tf.abs(gl_op)-e_param* elayerwise[vind])
        GLS = tf.sign(gl_op) * gl_plus
...
save_sparsity = []
#f=open(r'F:\\result\mnist2\SGL\using_param1.txt','w')
#f2=open(r'F:\\result\mnist2\SGL\test_accuracy1.txt','w')
#f3=open(r'F:\\result\mnist2\SGL\FLOP1.txt','w')
